...
import { ALL_CATEGORIES, CATEGORIES, DEFAULT_SLOT, SLOTS } from "@/lib/data/dimensions";
...
export default async function PetaPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const initialQuery: PetaInitialQuery = {
    stationId: Number.isFinite(stationValue) && stationValue > 0 ? stationValue : null,
    ...
    slot: SLOTS.some((item) => item.key === slotValue)
      ...
      : DEFAULT_SLOT,
    category: categoryValue === ALL_CATEGORIES || CATEGORIES.some((item) => item.key === categoryValue)
