...
test("cari stasiun lewat keyboard dan klik memindahkan kamera serta pilihan titik", async ({
  page,
}) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return {
          lng: Number(map.getCenter().lng.toFixed(5)),
          lat: Number(map.getCenter().lat.toFixed(5)),
          selected: map.getFeatureState({
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const map = (window as unknown as { __map: MapLibreMap }).__map;
        return (
          !map.isMoving() && Math.abs(map.getCenter().lng - 106.85005) < 0.00001
        );
      }),
    )
    .toBe(true);
  ...
  await page.evaluate(() =>
    (window as unknown as { __map: MapLibreMap }).__map.jumpTo({
      center: [107, -6],
    }),
  );
  await search.focus();
...
test("stasiun aktif tersimpan di URL dan pulih setelah refresh", async ({
  ...
  await expect(
    page.getByRole("combobox", { name: "Cari stasiun" }),
  ).toHaveValue("Sudirman");
  await page.getByRole("button", { name: "Retail", exact: true }).click();
  await expect(page.getByText(/Kios lokal Sudirman/)).toHaveCount(3);
  await expect(page.getByText("Famima Manggarai", { exact: true })).toHaveCount(
    0,
...
test("sepuluh lokasi retail dapat dipilih dari daftar dan marker, serta disembunyikan", async ({
  page,
}) => {
  await page.getByRole("button", { name: "Retail", exact: true }).click();
  ...
  await page
    .getByRole("button", { name: "Famima Manggarai", exact: true })
    .click();
    ...
    .poll(() =>
      page.evaluate(() => {
        ...
        return (
          map &&
          !map.isMoving() &&
          Math.abs(map.getCenter().lat + 6.2102422) < 0.000001 &&
          map.getZoom() === 19
        );
      }),
    )
  ...
  await page
    .getByRole("button", { name: "Indomaret Manggarai", exact: true })
    .click();
  await page
  ...
  await expect(detail).toContainText(
    "Estimasi potensi pendapatan belum tersedia",
  );
