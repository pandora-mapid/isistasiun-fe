...
> | **Indeks sewa / arus** | **Kotak petak sewa + angka indeks Rp/orang** |
...
>
> ~~Dua baris terakhir benar-benar kosong~~ — tinggal satu. *Indeks sewa / arus* sudah tersambung: dua sumber contoh baru (`rental-assets.json` + `rent-flow-index.json`) yang bentuknya menyamai response Go persis, digambar sebagai kotak petak (bukan bulatan — bulatan sudah dipakai dua arti) dengan angka indeks di bawahnya mulai zoom 16. Angkanya nyata, bukan karangan: petak Space KAI Manggarai (8 petak, 4 kosong No 5–8 @ Rp594 jt/thn) dibagi arus pintu terukur, dan kios Sudirman dari koordinat survei sendiri. *Event & aktivasi* masih benar-benar kosong: `/analytics/event-potential` ada di spesifikasi backend, tapi belum ada data contohnya.
>
...
| **Satu layar masih memakai sistem desain lama — sengaja** | Peta. Beranda, Insight, Metodologi, dan Rekomendasi sudah pindah ke sistem "laporan instrumen" (kertas hangat, tangga tipografi/ruang/radius yang sungguhan, biru dikunci untuk data) yang hidup berdampingan di `globals.css` lewat scope `.paper-canvas`. Peta ditinggal di sistem slate **dengan sengaja** — ia layar peta interaktif (satu-satunya client component, panel mengambang berkoordinat px, `.mono` untuk kolom angka berlebar tetap), bukan kandidat restyle seperti empat layar statis. Token lama masih dibaca `body`, `.page-canvas`, dan Peta, jadi jangan disatukan dengan mengubahnya. Dimigrasi satu layar per kali — Insight jadi contoh lengkapnya (primitif bersama ke `components/paper/`, daun data ke `components/insight/`, `globals.css` tak disentuh); Metodologi & Rekomendasi versi murahnya — restyle saja (`page-canvas` → `page-canvas paper-canvas`, hex slate → token paper, `.mono` → `.fig`, kepala → `Kepala`, kartu `#0F172A` → `.ink-band`, panel `#EEF2F6` → tint bergilir), layout & isi tetap, belum baca data hidup (Rekomendasi masih pakai label lama "Stasiun A/B/C" — menyambungkannya ke data dua-stasiun adalah pekerjaan terpisah). **Komposisinya bebas per layar**: Beranda majalah bersection (pita pastel full-bleed, numeral raksasa). Insight (putaran 4) memakai grid kartu **full-width dan padat** yang strukturnya dipinjam dari Metodologi/Rekomendasi — tanpa `.wrap`, section membentang selebar layar, tiap section grid berisi 2–3 kartu; latar kartu **bergilir menempuh mayoritas palet Beranda** (`--tile-sky/-mint/-violet/-rose`, `--data-wash`, `--field-wash`, `--paper-2`) di atas **satu latar `--paper` yang tidak pernah berganti warna** (itu batas antara "kartu bertint bergilir" dan "pemisahan halaman"), plus satu kartu hero `.ink-band`. Isi analitiknya: strip KPI, peringkat batang per pintu, pembacaan F×E×C×V, kolom "Menahan" di matriks. Tanpa pita full-bleed, `--tile-*` band, maupun `KickerBernomor`. (Tiga komposisi Insight lain — bersection ala Beranda, esai satu kolom, dan grid satu-panel-per-section yang terlalu lengang — sempat dibuat lalu ditolak.) Metodologi & Rekomendasi ikut rupa Insight (kartu datar bertint bergilir, kartu `.ink-band` gelap) sambil menahan layout aslinya |
